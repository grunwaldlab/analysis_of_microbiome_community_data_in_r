...
For example the graph below is from the publication our data set was used in:
...
For those who are color blind (~ 4% of people), there are even fewer colors that can be used. 
...
As with many other functions from `taxa` and `metacoder`, any table column or function assocaited with the `taxmap` object being plotted can be reffered to as if it was an independent variable in the function call.
...
We can force it to be the same each time by setting the seed for the random number generator used by R, as is done in the next example.
...
Up until now, we have set both the color and size to display the number of OTUs, but they can be set to differnt things.
Lets set the color to the number of supertaxa that each taxon is included in.
...
Note how both sides of the legned on the bottom right have numbers now.
...
However, we need some more things to graph, so we will calculate the number of read depth and number of samples for each taxon.
...
Since the "leaf" and "root" columns have one value per taxon, they can be used from plotting.
...
You can add a title to the graph and change the legend labels to make the graph easier to understand.
...
These accept two or more colors, either as common color names (e.g. `c("red", "green")`) or hexadecimal color codes (e.g. `#000000` is black).
...
Common examples include "root", "bacteria", or "cellular organisms".
When a taxonomy has multiple roots, multiple trees are graphed in the same plot.
We force multiple roots to demonstrate this by selecting some taxa without preserving supertaxa:
...
This can be used to make very large taxonomies easier to understand or emphaize the differenced between taxa.

## Exercises

**1a)** Make a plot of *Firmicutes* and subtaxa with the size of nodes proportional to the number of OTUs and label the nodes with taxon names.
...
**1b)** Modify the plot you made in part **a** so that the taxon names and OTU counts are combined in the label like "my_taxon (123)". Hint: try `paste0` to combine multiple peices of text.
...
Remake the graph for **1a**, but use `node_size_range` to make the node sizes vary between 0.5% and 5% of the graph width.
...
**3a)** Using the "root" and "leaf" columns in the "tax_abund" table created earlier in this section for this example.
Plot the number of reads in leaf samples in "Proteobacteria" and subtaxa using the size and color of nodes.

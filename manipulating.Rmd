...
Depending on the format the data is in, this might be easy or hard.
If other data  (e.g. OTU counts) are mapped to taxa within that phylum, do we also remove those?
After all, even if the phylum and the taxa within it are removed, we still know that all the data that were mapped to them are bacteria of some kind, so do we reassign them to the Bacteria taxon, or just throw those data out?

The answer to these kind of questions will depend of our data and what we want to do with it.
...
In the next section we will do some more filtering  on this data set, but we will focus on standard quality control specific to amplicon sequence data.

...
`obj$data` is simply a list of arbitrary, user-defined data sets.
...
If you are not familiar with using regular expressions, this might be challenging to understand at first, but it is a very versatile skill to have, so it is worth learning.
...
* The `^` and `$` represent the start and end of the string respectively. If these were not there, then the pattern could match just a part of the string.
...
"From the start of the string, (`^`) match any character between "a" and "z" (`[a-z]`) zero or one times (`{0,1}`) followed by an underscore (`_`) that occurring between zero and 2 times, followed by any character (`.`) occurring zero or more times (`*`), followed by the end of the string (`$`)."
...
In the next section we will work on filtering this data set.

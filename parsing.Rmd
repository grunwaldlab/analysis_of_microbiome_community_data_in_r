...
If are not familiar with using regular expressions, this might be challenging to understand at first, but it is a very versatile skill to have, so it is worth learning 
...
The square brackets (e.g. `[a-z]`) specify a range of characters that can be matched. 
Likewise the `.` means match any character.
...
For each capture group, the is a value given to the `class_key` option specifying what the group is (e.g. taxon name).
...
Instead, the rank information is in a separate data set:

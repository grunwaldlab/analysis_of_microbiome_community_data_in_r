...
If something does not make sense now, dont worry!
...
`Metacoder` has functions for parsing specific file formats used in metagenomics research.
...
It can be calculated at any rank of the taxonomy, but it is usually calulated at the species or OTU "rank".
...
Thats more interesting; skin has much lower diversity than are of the wetter areas, which makes sense.
Lets see if thats a significant difference using an ANOVA.
...
With a little tweeking, we can add this data to the graph we made.
...
This tells us that samples from the nose, stool, and throat have no significant differences in alpha diversity of OTUs, but samples from siliva and skin are signifcantly different from the rest. 

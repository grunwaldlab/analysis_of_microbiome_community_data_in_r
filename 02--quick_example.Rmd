...
* A count matrix called `hmp_otus`, with **samples in columns** and **OTUs in rows**
* A sample data table called `hmp_samples`, with **samples as rows** and columns of information describing the samples (e.g., gender)

> Because our example only has two data sources should omit one of these graphics?

> The graphic is labeled 'Count data' but you refer to this in text as 'abundance matrix'. I think standardizing this will bring greater clarity.
...
We can process the count matrix, and parse the taxonomic information at the same time, using a parser from `taxa`.
...
Note how the original count matrix is contained in the `taxmap` object with an additional column called "taxon_id".
...
An OTU may contain sequence variants.
These OTUs are assigned to taxa so a taxon may include multiple OTUs.
Taxa may also include different numbers of ranks.
For example, one taxon may include species while another only goes to genus.
These abstractions are necessary becuase we may have sequences that we can not confidently assign to a traditional taxon.
...
Each cell is the number of times an OTU (a sequence) was observed in a sample.
...
By setting low abundance cells to zero we may have created OTUs that no longer contain any observations.
...
In a perfect world we would sequence each sample the same amount (i.e., the same number of reads).
We do not live in a perfect world, so some samples may have more reads than others.
...
So far we've used raw counts, but people typically work with rarefied counts or proportions to try to avoid the possibility of sampling depth biasing the results.

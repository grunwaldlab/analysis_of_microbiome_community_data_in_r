...
The `zero_low_counts` counts can be used to convert any conts less than some number to zero.
...
Rarefaction is used to simulate even numebrs of samples (i.e. reads).
...
However, since there is now two tables, we should not remove any taxa since they still might be used in the unrarefied table, so we will not add the `drop_taxa = TRUE` option this time.
...
Each line represents a different sample (i.e. column) and shows how many OTUs are found in a random subsets of differnent numbers of reads.
...
If you plot all the samples, which takes a few mintues, it looks like this:
...
Therefore, insead of comparing read counts, the counts can be converted to presence or absence of an OTU in a given sample.
...
**1c)** What would be a sufficent read count to capture all of the diverstiy in these three samples?
